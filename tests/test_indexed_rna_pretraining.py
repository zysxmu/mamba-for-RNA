...
from scripts.prepare_pretraining_6m import prepare_corpus
...
def _synthetic_bundle(path) -> None:
    euk = _zip_bytes(
        {
...
def test_prepare_and_read_indexed_six_source_corpus(tmp_path):
    ...
    assert manifest["totals"]["records"] == 13
    assert manifest["totals"]["source_class_counts"] == {"ncRNA": 4, "coding": 9}
    assert manifest["corpus_contract"]["coding_primary_records"] == 5
    assert manifest["corpus_contract"]["coding_cds_view_fillers"] == 4
    assert manifest["corpus_contract"]["m6a_used"] is False
    assert sum(item["records"] for item in manifest["splits"].values()) == 13
